...
{
    private string[] GetColName(Dictionary<string, string> colNames ,string name)
    {
        if (colNames==null || colNames.Count==0) return name.ToLower().Split("|");
        if (colNames.ContainsKey(name)) return colNames[name].ToLower().Split("|");
        return name.ToLower().Split("|"); 
    }
    public async Task<IList<T>> ToListAsync<T>(FileUploadRequest request, FileType supportedFileType, string sheetName = "Sheet1",Dictionary<string,string> colNames=null)
    {
        ...
        using (IXLWorkbook workbook = new XLWorkbook(streamData))
        {
            ...
            var worksheet = workbook.Worksheets.FirstOrDefault(w => w.Name.Contains(sheetName));
            if (worksheet != null)
            {
                ...
                foreach (IXLRow row in worksheet.RowsUsed().Skip(1))
                {
                    ...
                    foreach (var prop in properties)
                    {
                        try
                        {
                            ...
                            var col = columns.SingleOrDefault(c => GetColName(colNames, prop.Name).Contains(c.Value.ToString().ToLower()));
